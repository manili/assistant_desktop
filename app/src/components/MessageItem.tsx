...
export const MessageItem = React.memo(function MessageItem({
  ...
  return (
    ...
    >
      {/* 1. History Context Selection Checkbox (Checked-Only / Hover-Only) */}
      {onToggleMessageSelect && (
        <div className="absolute left-2 top-2 z-10 select-none">
          <input
            type="checkbox"
            checked={isSelected}
            onChange={(e) => onToggleMessageSelect(index, e.target.checked)}
            className={`rounded border border-slate-700 bg-slate-950 text-indigo-500 focus:ring-indigo-500/30 focus:ring-offset-slate-900 focus:ring-2 w-3.5 h-3.5 cursor-pointer transition-all duration-150 ${
              isSelected
                ? "opacity-100 scale-100"
                : "opacity-0 scale-95 group-hover:opacity-100 group-hover:scale-100"
            }`}
            title={
              isSelected ? "Included in AI context" : "Excluded from AI context"
            }
          />
        </div>
      )}
          ...
          >
            ...
            >
              <path
                ...
                d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 002-2M8 5a2 2 0 112-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3"
              />
            </svg>
          </button>
          <button
      ...
      {/* Extra left padding to offset the context selection checkbox cleanly */}
      <span className="text-xs font-bold uppercase mb-1 block text-slate-500 pl-4">
        {msg.role}
      </span>
      ...
      <div className="pl-4">
        {isEditing ? (
            ...
            {parseAgentOutput(msg.content).map((block, idx) => {
              if (block.type === "text") {
                return (
                  ...
                  >
                    {renderMarkdown(block.content)} {/* Compiled Markdown */}
                  </div>
                );
                ...
                return (
                  ...
                  >
                    ...
                    <code className="block bg-slate-950 p-2 rounded text-xs font-mono text-slate-200 overflow-x-auto whitespace-pre selection:bg-indigo-800">
                      {block.content}
                    </code>
          ...
          <div className="font-sans text-slate-300 leading-relaxed">
            {renderMarkdown(msg.content)} {/* Compiled Markdown */}
          </div>
        )}
      </div>
    </div>
